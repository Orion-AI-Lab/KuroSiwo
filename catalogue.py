...
def _parseCFG():
    ...
    _pp = dict(act_id= str, act_region= str, ref_date= lambda v: ciso8601.parse_datetime(v),
...
class GridProduct(object):
    ...
    def todict(self):
        info = dict(name= self.name, master=self.master, nodata=self.nodata, dtype=self.dtype,
                    ptype=self.ptype, pname=self.pname, rank=self.rank, source_date= self.source_date,
                    clz_id=self.clz_id, clz_name=self.clz_name,
    ...
    def __init__(self, gd=None, base=None, pname=None):
        ...
        try:
            self._aoi_meta = CFG['Floods'][f'{self.gd.actid}']['aois'][f'{self.gd.aoiid:02d}']
        except:
            if self.gd.aoiid is None:
                self.gd.aoiid=999999
            self._aoi_meta = dict(aoi_id=f'{self.gd.aoiid:02d}',aoi_name=None, cl_zone=None, cl_name=None)
...
class CatalogueGrid(object):
    ...
    def path(self):
        if self.inaoi:
            pargs = [str(self.gdmaster['actid']),'{:02d}'.format(self.gdmaster['aoiid']), self.gdmaster['grid_id'].hex]
        else:
            pargs = [str(self.gdmaster['actid']), '00', self.gdmaster['grid_id'].hex[0:2], self.gdmaster['grid_id'].hex]
    ...
    def __init__(self, ggdf):
        ...
        self.gdslave = [row for _, row in self.gdf.loc[self.gdf['master'] == False].iterrows()]

        self.infopath = self.path.joinpath('info.json')
        for ptype in ('IVH','IVV'):
            for gd in ([self.gdmaster,] + self.gdslave):
        ...
        if self.inaoi:
            self.datasets.append(GridProduct(gd=self.gdmaster,base=self.path, pname='MLU'))
...
class Catalogue(object):
    _schema = {'geometry': 'Polygon', 'properties': OrderedDict([
        ('grid_id', 'str'), ('slavecov', 'int'), ('mastercov', 'int'), ('gvalid', 'bool'),
        ('pcovered', 'float'), ('pwater', 'float'), ('pflood', 'float'), ('actid', 'int'), ('flood_date', 'datetime'),
        ('aoiid', 'int'), ('revision', 'int'), ('version', 'int'),
        ('source_date', 'date'), ('s1_ids', 'str'), ('master', 'bool'), ('coverage', 'float'), ('crank', 'int')
    ])}

    _schema_ext = {'geometry': 'Polygon', 'properties': OrderedDict([
        ('grid_id', 'str'), ('slavecov', 'int'), ('mastercov', 'int'), ('gvalid', 'bool'),
        ('pcovered', 'float'), ('pwater', 'float'), ('pflood', 'float'), ('actid', 'int'), ('flood_date', 'datetime'),
        ('aoiid', 'int'), ('revision', 'int'), ('version', 'int'),
        ('source_date', 'date'), ('s1_ids', 'str'), ('master', 'bool'), ('coverage', 'float'), ('crank', 'int'),
        ('exported', 'bool'), ('shuffle', 'int')
    ])}
    ...
    @property
    def aoiids(self):
        uv = [_ for _ in list(self.gdf.drop_duplicates(['actid', 'aoiid'])[['actid', 'aoiid']].to_records(index=False)) if _[1] is not None]
        for uv_ in uv:
            yield uv_

    def filter(self, pcovered=None, ppermwater=None, pflooded=None, pwater=None, coverage='ALL'):
        gdf_= self.gdf
        ...
        gdf_.sort_values(by=['actid', 'aoiid', 'grid_id', 'master', 'crank'], ascending=True, inplace=True)
        fcat = Catalogue(gdf_, schema=self.Schema.extended)
        stats_ = dict(records=len(fcat),
                      activations=len(list(fcat.actids)),
                      actids= list(fcat.actids),
                      aois=len(list(fcat.aoiids)), grids=len(list(fcat.ids)),
                      coverage=coverage)

        if coverage=='AOI':
            statsext_ = dict(mean_pcovered=fcat.gdf[fcat.gdf['pcovered'] != None]['pcovered'].mean(),
    ...
    def write(self, path):
        gdf = self.gdf.copy()
        gdf['grid_id'] = self.gdf['grid_id'].apply(lambda c: str(c) if c is not None else c)
        gdf['s1_ids'] = self.gdf['s1_ids'].apply(lambda c: json.dumps([str(_) for _ in c]) if c is not None else c)
        self.schema_ = self._schema_ext if 'exported' in gdf.columns else self._schema
        gdf.sort_values(by=['actid', 'aoiid', 'grid_id', 'master', 'crank'], ascending=True, inplace=True)
    ...
    def read(self, path, schema=None):
        ...
        gdf['grid_id'] = gdf['grid_id'].apply(lambda c: uuidmod.UUID(c) if c is not None else c)
        gdf['s1_ids'] = gdf['s1_ids'].apply(lambda c: [uuidmod.UUID(_) for _ in json.loads(c)] if c is not None else c)
        gdf['flood_date'] = gdf['flood_date'].apply(lambda c: ciso8601.parse_datetime(c) if c is not None else c)
        gdf['source_date'] = gdf['source_date'].apply(lambda c: ciso8601.parse_datetime(c).date() if c is not None else c)
    ...
    def __init__(self, gdf=None, schema=None):
        self.gdf = gdf
        self._schemamap = {k.name:v for k,v in zip((self.Schema.simple,self.Schema.extended),(self._schema,self._schema_ext))}
...
def create_parser():
    ...
    def oper(args):
        cpath = Path(CFG['CAT_PATH']) if not args.catalogue_path else Path(args.catalogue_path)
        dpath = Path(CFG['REPO_PATH']) if not args.data_path else Path(args.data_path)

        if not cpath.exists() and cpath.is_file():
            raise FileNotFoundError(f'Provided catalogue dataset {cpath.as_posix()} does not exist')
        if not dpath.exists() and dpath.is_file():
            raise FileNotFoundError(f'Provided datasets repository {dpath.as_posix()} does not exist')
        ...
        for f in ('pcovered','ppermwater','pflooded','pwater'):
            ...
            if v:
                v_ = json.loads(v)
                assert isinstance(v_, list) and len(v_)==2 and 0<=v_[0]<=100 and 0<=v_[1]<=100, 'Range value error, use template e.g: [0,100]'
                setattr(args, f, v_)
        ...
        if args.coverage!='AOI':
            ...
            args.pwater = None
        fcat = cat.filter(pcovered=args.pcovered,ppermwater=args.ppermwater,pflooded=args.pflooded,pwater=args.pwater, coverage=args.coverage)
        ...
        for i, id in enumerate(fcat.ids):
            ...
            grid_dict[gridd.id.hex] = {'path': relative_path, 'info': gridd.info,'clz':gridd.clz_id,'clz_name':gridd.clz_name}
    ...
    _parser = argparse.ArgumentParser(prog='NNFloods_oper', description='NNFloods Catalogue Operations')
    _parser.set_defaults(func=oper)

    _parser.add_argument('--version', action='version', version='1.0.0')
    _parser.add_argument('--catalogue-path', type=str, help='Merged catalogues path or <CAT_PATH>@CONF')
    _parser.add_argument('--data-path', type=str, help='Root datasets path or <REPO_PATH>@CONF')

    fg = _parser.add_argument_group(title='Filters Group', description='Define Filters with <AND> logic')
    fg.add_argument('-a', '--activations', nargs='+', required=False, default='ALL', type=str, action='store',
                    choices=list(CFG["Floods"].keys()) + ['ALL', ], help='Filter activations, DEFAULT: %(default)s')
    fg.add_argument('-c', '--coverage', nargs='?', required=False, default='ALL', type=str, action='store',
                    choices=['ACV','AOI','ALL'],
                    help='Filter by coverage, DEFAULT: %(default)s, ACV-> Only out of AOI Areas, AOI-> only on AOI areas, ALL-> Both areas')
    fg.add_argument('-pv', '--pcovered',type=str, required=False, default='[40,100]', help='Valid percentage of grid, DEFAULT: %(default)s')
    fg.add_argument('-pw', '--ppermwater', type=str, required=False, default='[0,100]', help='Percentage of grid with permanent water, DEFAULT: %(default)s')
    fg.add_argument('-pf', '--pflooded', type=str, required=False, default='[0,100]', help='Percentage of grid with flooded water, DEFAULT: %(default)s')
    fg.add_argument('-pW', '--pwater', type=str, required=False, default='[0,100]', help='Total percentage of grid water (Permanent+Flooded), DEFAULT: %(default)s')

    _parser.add_argument('-i', '--info', action='store_true', help='Display info on filtered data')
